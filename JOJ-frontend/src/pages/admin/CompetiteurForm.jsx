        ...
        <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 mt-8">
          ...
          <div className="flex justify-start mb-8">
            <div className="bg-gray-100 rounded-full p-1 flex">
              <button 
                onClick={() => setTeamType('individuel')} 
                className={`px-6 py-2 rounded-full text-sm font-medium transition-colors ${teamType === 'individuel' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-600 hover:text-gray-900'}`}
              >
                Individuel
              </button>
              <button 
                onClick={() => setTeamType('collectif')} 
                className={`px-6 py-2 rounded-full text-sm font-medium transition-colors ${teamType === 'collectif' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-600 hover:text-gray-900'}`}
              >
                Collectif
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="mb-6">
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-900 mb-2">
                      Logo de l'équipe ou photo du joueur
                    </label>
              ...
              <div className="mb-6">
                ...
                <select 
                  ...
                  onChange={handleInputChange}  
                  className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent bg-white"
                >
              ...
              <div className="mb-8">
                ...
                <button 
                  ...
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${formData.status ? 'bg-orange-600' : 'bg-gray-200'}`}
                >
              ...
              <div className="flex justify-end space-x-3">
                ...
                <button 
                  type="submit" 
                  disabled={loading} 
                  className={`px-6 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition-colors font-medium ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
                > 
