...
// QR Code unique par billet : les modules de données varient selon billet.qrSeed

// Positions fixes des modules de données (hors patterns de détection)
...
// Filtre les positions selon le seed binaire du billet → QR unique
function genererModules(seed) {
  return POSITIONS_BASE.filter((_, i) => seed[i % seed.length] === 1);
}

const CarteQR = ({ billet, onTelecharger, onAccueil }) => {
  const modules = genererModules(billet.qrSeed);
    ...
    <div className="bg-white rounded-3xl shadow-lg p-8 flex flex-col items-center">
      ...
      <div className="bg-white p-4 mb-6">
        <svg viewBox="0 0 200 200" className="w-48 h-48">
          ...
          {/* Patterns de détection (identiques sur tous les QR — standard) */}
          ...
          {/* Modules de données uniques selon le seed */}
          {modules.map(([x, y], i) => (
      ...
      <p className="text-gray-400 text-xs font-mono mb-2">{billet.id}</p>
